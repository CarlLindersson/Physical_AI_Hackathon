...
GRAB_LIFT_M = 0.120
GRAB_LIFT_STEPS = 4
DEFAULT_LIFT_JOINT_DEG = 20.0
DEFAULT_LIFT_JOINT_SIGN = 1.0
DEFAULT_MIN_CARRY_Z_M = 0.180
...
DEFAULT_INTRINSIC_CALIBRATION = "run/intrinsic_calibration.json"
...
DEFAULT_ZONE_A_DROP_BASE_DEG = -90.0
DEFAULT_ZONE_B_DROP_BASE_DEG = 90.0
DEFAULT_ZONE_C_DROP_BASE_DEG = -135.0
DEFAULT_AUTO_REPEAT_GRAB = True
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=os.environ.get("INTRINSIC_CALIBRATION", DEFAULT_INTRINSIC_CALIBRATION),
        help="Optional fisheye intrinsic JSON from run/calibrate_fisheye_intrinsics.py.",
    )
    parser.add_argument(
        "--no-intrinsic-undistort",
        action="store_true",
        help="Use raw detection pixels even if an intrinsic calibration file exists.",
    )
    parser.add_argument(
        "--vision-backend",
    ...
    parser.add_argument(
        ...
        help="Minimum relative lift distance in meters after the grip is complete.",
    )
    parser.add_argument(
        "--lift-joint-deg",
        type=float,
        default=float(os.environ.get("LIFT_JOINT_DEG", str(DEFAULT_LIFT_JOINT_DEG))),
        help="Shoulder/base-lift joint raise in degrees after the gripper closes.",
    )
    parser.add_argument(
        "--lift-joint-sign",
        type=float,
        choices=(-1.0, 1.0),
        default=float(os.environ.get("LIFT_JOINT_SIGN", str(DEFAULT_LIFT_JOINT_SIGN))),
        help="Use 1 when increasing the shoulder/base-lift joint raises the object; use -1 if reversed.",
    )
    parser.add_argument(
        "--grab-lift-steps",
        type=int,
    ...
    parser.add_argument(
        "--min-carry-z-m",
        type=float,
        default=float(os.environ.get("MIN_CARRY_Z_M", str(DEFAULT_MIN_CARRY_Z_M))),
        help="Minimum absolute Z height used before rotating toward the drop zone.",
    )
    parser.add_argument(
        "--grab-close-ticks",
        type=int,
    ...
    parser.add_argument(
        "--zone-a-drop-base-deg",
        type=float,
        default=float(os.environ.get("ZONE_A_DROP_BASE_DEG", str(DEFAULT_ZONE_A_DROP_BASE_DEG))),
        help="Base yaw angle used to drop Zone A objects after lifting.",
    )
    parser.add_argument(
        "--zone-b-drop-base-deg",
        type=float,
        default=float(os.environ.get("ZONE_B_DROP_BASE_DEG", str(DEFAULT_ZONE_B_DROP_BASE_DEG))),
        help="Base yaw angle used to drop Zone B objects after lifting.",
    )
    parser.add_argument(
        "--zone-c-drop-base-deg",
        type=float,
        default=float(os.environ.get("ZONE_C_DROP_BASE_DEG", str(DEFAULT_ZONE_C_DROP_BASE_DEG))),
        help="Base yaw angle used to drop Zone C/other objects after lifting.",
    )
    parser.add_argument(
        "--no-auto-repeat-grab",
        action="store_true",
        default=not parse_bool_env(os.environ.get("AUTO_REPEAT_GRAB"), DEFAULT_AUTO_REPEAT_GRAB),
        help="Disable automatic restart of the g pick loop after each drop.",
    )
...
def lift_joint_sign(args):
    return -1.0 if args.lift_joint_sign < 0 else 1.0
...
def load_calibration_map(args):
    ...
    intrinsics = None if args.no_intrinsic_undistort else load_intrinsic_calibration(args)
    return {
        "path": path,
        "transform": transform,
        "table_z": table_z,
...
def load_intrinsic_calibration(args):
    path = resolve_repo_path(args.intrinsic_calibration)
    if not path.exists():
        print(f"Intrinsic calibration not found: {path}")
        print("         Using raw detection pixels for table transform.")
        return None
    ...
    try:
        ...
        print(f"Warning: could not load intrinsic calibration {path}: {exc}")
        print("         Using raw detection pixels for table transform.")
        return None

    if k.shape != (3, 3) or d.shape[0] < 4:
        print(f"Warning: intrinsic calibration has invalid K/D shapes: {path}")
        print("         Using raw detection pixels for table transform.")
        return None
    ...
    if width and height and (int(width), int(height)) != (CAMERA_WIDTH, CAMERA_HEIGHT):
        print(
            "Warning: intrinsic calibration resolution "
            f"{width}x{height} differs from runtime {CAMERA_WIDTH}x{CAMERA_HEIGHT}."
        )

    print(f"Intrinsic fisheye calibration loaded: {path}")
    return {
        "path": path,
        "K": k,
        "D": d,
...
def undistort_pixel_xy(pixel_xy, calibration):
    intrinsics = calibration.get("intrinsics") if calibration else None
    if intrinsics is None:
        return np.asarray(pixel_xy, dtype=np.float64), "raw"
    ...
    try:
        ...
        print(f"Warning: fisheye undistort failed; using raw pixel: {short_text(exc, 80)}")
        return np.asarray(pixel_xy, dtype=np.float64), "raw"

    return undistorted.reshape(2), "undistorted"
...
def pixel_to_robot_xyz(pixel_xy, calibration, args):
    ...
    mapped_pixel, pixel_source = undistort_pixel_xy(pixel_xy, calibration)
    u, v = [float(value) for value in mapped_pixel[:2]]
    ...
    return np.array([xy[0], xy[1], z], dtype=np.float64), mapped_pixel, pixel_source
...
def zone_drop_base_angle(zone, args):
    if zone == "ZONE_A":
        angle_deg = args.zone_a_drop_base_deg
    elif zone == "ZONE_B":
        angle_deg = args.zone_b_drop_base_deg
    else:
        angle_deg = args.zone_c_drop_base_deg

    angle_rad = np.deg2rad(angle_deg)
    return float(np.clip(angle_rad, QArmMini.LIMITS_MIN[0], QArmMini.LIMITS_MAX[0]))
...
class GrabController:
    ...
    def update(self, joint_cmd, gripper_cmd, vision, timestep, args, arm_math, selected_target_index):
        ...
        if self.state == "move_lift":
            return self._update_joint_lift(joint_cmd, timestep, args)

        if self.state == "rotate_drop":
            return self._update_rotate_drop(joint_cmd, timestep, args)
    ...
    def _build_plan(self, detection, args, arm_math, joint_cmd):
        ...
        try:
            object_xyz, mapped_px, pixel_source = pixel_to_robot_xyz(center_px, self.calibration, args)
        except Exception as exc:
            self._fail(str(exc))
            return False
        ...
        self.plan = {
            ...
            "zone": zone,
            "drop_base_angle_rad": zone_drop_base_angle(zone, args),
            "center_px": center_px,
            "mapped_px": mapped_px,
    ...
    def _update_joint_lift(self, joint_cmd, timestep, args):
        ...
        self.status = (
            "raise lift joint "
            f"{np.rad2deg(joint_cmd[1]):.1f}->{np.rad2deg(lift_target[1]):.1f} deg"
        )
        if reached:
    ...
    def _prepare_lift_joint_target(self, joint_cmd, args):
        if not self.plan:
            return
        lift_joint_target = np.asarray(joint_cmd, dtype=float).copy()
        lift_joint_delta = lift_joint_sign(args) * np.deg2rad(abs(args.lift_joint_deg))
        lift_joint_target[1] += lift_joint_delta
        np.clip(lift_joint_target, QArmMini.LIMITS_MIN, QArmMini.LIMITS_MAX, out=lift_joint_target)
        self.plan["lift_joint_target"] = lift_joint_target
        print(
            "Lift joint target: "
            f"shoulder {np.rad2deg(joint_cmd[1]):.1f}->{np.rad2deg(lift_joint_target[1]):.1f} deg"
        )
    ...
    def _enter(self, state, status, joint_cmd=None):
        ...
        if state == "grip_settle":
            ...
            self.open_ticks = 0
            self.grip_hold_joint_cmd = None
        elif state == "rotate_drop":
            zone = self.plan["zone"] if self.plan else "ZONE_C"
            self.drop_joint_target = joint_cmd.copy() if joint_cmd is not None else wrist_down_home_pose()
            if self.plan and "drop_base_angle_rad" in self.plan:
                self.drop_joint_target[0] = self.plan["drop_base_angle_rad"]
            np.clip(self.drop_joint_target, QArmMini.LIMITS_MIN, QArmMini.LIMITS_MAX, out=self.drop_joint_target)
            self.grip_hold_joint_cmd = None
        else:
            self.grip_hold_joint_cmd = None
